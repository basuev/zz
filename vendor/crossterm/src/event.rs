...
pub struct KeyEvent {
    /// The key itself after applying the active keyboard layout.
    pub code: KeyCode,
    /// The key at the same physical position in the standard PC-101 layout, when
...
impl PartialEq for KeyEvent {
    fn eq(&self, other: &KeyEvent) -> bool {
        ...
        let KeyEvent {
            code: rhs_code,
            base_code: rhs_base_code,
            modifiers: rhs_modifiers,
            kind: rhs_kind,
            state: rhs_state,
        } = other.normalize_case();
        (lhs_code == rhs_code)
            && (lhs_base_code == rhs_base_code)
