...
pub(crate) fn parse_csi_u_encoded_key_code(buffer: &[u8]) -> io::Result<Option<InternalEvent>> {
    ...
    let codepoint = codepoints
        ...
        .parse::<u32>()
        .map_err(|_| could_not_parse_event_error())?;
    let shifted_key = codepoints
        .next()
        .and_then(|codepoint| codepoint.parse::<u32>().ok())
        .and_then(char::from_u32);
    let base_code = codepoints
        .next()
        .and_then(|codepoint| codepoint.parse::<u32>().ok())
        .and_then(char::from_u32)
        .map(KeyCode::Char);
    ...
    if modifiers.contains(KeyModifiers::SHIFT) {
        if let Some(shifted_c) = shifted_key {
            ...
            // Keep Shift when a base-layout key is available so consumers can
            // reconstruct the shifted physical key independently of layout.
    ...
    key_event.base_code = base_code;
    let input_event = Event::Key(key_event);
